...
class AudioContextWrapper {
  ...
  constructor(defaultParameters: IAudioContextParameters) {
    ...
    const browserCompatibleAudioContext =
    ...
    this.noiseOsc = new WhiteNoiseOscillator(
      this.audioContext,
      noiseGain,
      this.filterNode
    );
  ...
  playNote(note: keyNames) {
    ...
    if (!voice) {
      console.log(
        "Unable to find voice with note " + note + " and octave " + this.octave
      );
      return;
    }
  ...
  stopNote(note: string) {
    ...
    this.numPlayingVoices =
      this.numPlayingVoices - 1 < 0 ? 0 : this.numPlayingVoices - 1;
  ...
  findOscWithNoteAndOctave(voiceList: Voice[], note: string) {
    return voiceList.find(
      (osc) => osc.octave == this.octave && osc.note == note
    );
  }
}
